...
function parse(path) {
  ...
  // Parse subexpressions, functions and breakets

  function recurse(remainder, i) {
    var result, previous = remainder[i-1] || {};
    if(remainder[i].operator === '[') {
      if(!previous.identifier) throw "Unexpected [";
      result = foldSubExpression(remainder.slice(i+1));
      previous.identifier = previous.identifier.concat(result.rest);
      remainder.splice(i, result.remove);
    } else if(remainder[i].operator === '(') {
      if(previous.identifier || previous.call) {
        result = getCallParams(remainder.slice(i+1));
        remainder[i-1] = {call: previous, args: result.args};
        remainder.splice(i, remainder.length - (result.rest + i));
        i--;
      } else {
        result = getBreaketContent(remainder.slice(i+1));
        remainder[i] = {breaket: result.content};
        remainder.splice(i+1, remainder.length - (result.rest + i + 1));
      }
    }
  }

  function getBreaketContent(remainder) {
    for(var i = 0; i < remainder.length; i++) {
      if(remainder[i].operator === ')') {
        return {rest: remainder.length - i - 1, content: remainder.slice(0,i)};
      }
      recurse(remainder, i);
    }
  ...
  function getCallParams(remainder) {
    var args = [], current = 0;
    for(var i = 0; i < remainder.length; i++) {
      if(remainder[i].operator === ')' || remainder[i].operator === ',') {
        if(i > 0) {
          args.push(remainder.slice(current, i-1));
          current = i+1;
        }
      }
      if(remainder[i].operator === ')') {
        return {rest: remainder.length - i - 1, args: args};
      }
      recurse(remainder, i);
    }
    throw "Unexpected ending of path";
  ...
  function foldSubExpression(remainder) {
    for(var i = 0; i < remainder.length; i++) {
